...
uint8_t Memoire24::m_adresse_peripherique = 0xA0;

/******************************************************************************/
/* void Memoire24::Memoire24()                                        */
/*                                                                            */
...
/******************************************************************************/
/* uint8_t Memoire24::choisir_banc(const uint8_t banc)                    */
/*                                                                            */
...
uint8_t Memoire24::choisir_banc(const uint8_t banc)
{
   ...
   if(banc == temp)
   {
      Memoire24::m_adresse_peripherique = (0xA0 | ( banc << 1 ));
      rv = Memoire24::m_adresse_peripherique;
   }
   return rv;
}
...
/*                                                                            */
/******************************************************************************/
uint8_t Memoire24::lecture(const uint16_t adresse, uint8_t *donnee)
{
  uint8_t rv = 0;
...
/*                                                                            */
/******************************************************************************/
uint8_t Memoire24::ecriture(const uint16_t adresse, const uint8_t donnee)
{
  //______________ Attente de la fin d'un cycle d'ecriture ______________
  while (true)
  {
...
uint8_t Memoire24::ecriture(const uint16_t adresse, uint8_t *donnee,
...
uint8_t Memoire24::ecrire_page(const uint16_t adresse, uint8_t *donnee,
                                   const uint8_t longueur)
{
  uint16_t addr_fin;
  uint8_t rv = 0;
...
uint8_t Memoire24::lire(uint8_t *donnee)
{
   uint8_t rv = 0;
   ...
   //______________ Transmission du poids fort de l'adresse ________________
   TWDR =  ( adresseLecture_ >> 8 );            // 8 bits de poids fort de l'addresse
   TWCR = _BV(TWINT) | _BV(TWEN);       // R. a Z., interrupt. - Depart de transm.
   while ((TWCR & _BV(TWINT)) == 0)     // Attente de fin de transmission
      ;

   //_____________ Transmission du poids faible de l'adresse _______________
   TWDR = adresseLecture_;                      // 8 bits de poids faible de l'addresse
   TWCR = _BV(TWINT) | _BV(TWEN);       // R.�Z., interrupt. - Depart de transm.
   while ((TWCR & _BV(TWINT)) == 0)     // Attente de fin de transmission
      ;
...
uint8_t Memoire24::ecrire(const uint8_t donnee)
{
   //______________ Attente de la fin d'un cycle d'ecriture ______________
   while (true)
   {
   ...
   //______________ Transmission du poids fort de l'adresse ________________
   TWDR = adresseEcriture_ >> 8;                 // 8 bits de poids fort de l'adresse
   TWCR = _BV(TWINT) | _BV(TWEN);        // R. a Z., interrupt. - Depart de transm.
   while ((TWCR & _BV(TWINT)) == 0)      // Attente de fin de transmission
      ;

   //_____________ Transmission du poids faible de l'adresse _______________
   TWDR = adresseEcriture_;                      // 8 bits de poids faible de l'adresse
   TWCR = _BV(TWINT) | _BV(TWEN);        // R. a Z., interrupt. - Depart de transm.
   while ((TWCR & _BV(TWINT)) == 0)      // Attente de fin de transmission
      ;
...
void Memoire24::changerAdresseLecture(uint16_t adresse)
{
   adresseLecture_ = adresse;
}

void Memoire24::changerAdresseEcriture(uint16_t adresse)
{
   adresseEcriture_ = adresse;
}
